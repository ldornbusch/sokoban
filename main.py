...
WINDOW_WIDTH = 640  # size of the window displayed on screen
WINDOW_HEIGHT = 480

GAME_WIDTH = 640.0  # internal game logic resolution
GAME_HEIGHT = 480.0

TPS = 60  # number of game updates per second
SKIP_TICKS = 1000 / TPS  # ms to start skipping frames
MAX_FRAME_SKIP = 5  # no we calc max updates (if we are behind) before displaying
MAX_TIME_SKIP = 1000  # max Time we try to catch up until we just reset counter
...
def gameloop():  # http://www.koonsolo.com/news/dewitters-gameloop/
    ...
    while 1:
        loops = 0
        while get_real_time() > next_game_tick and loops < MAX_FRAME_SKIP:
            ...
            frame_counter += 1
            sokoban_game.handle_event(frame_counter)
            next_game_tick += SKIP_TICKS
            if get_real_time() > next_game_tick + MAX_TIME_SKIP:
                next_game_tick = get_real_time() + SKIP_TICKS
            loops += 1

        screen.fill(0)
        sokoban_game.paint(frame_counter)
